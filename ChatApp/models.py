...
# 2024/11/23 yoneyama add start
    def getGroup(groupname):
        try:
            conn = DB.getConnection()
            cur = conn.cursor()
            #sql = "SELECT c.cid ,c.name ,c.required ,c.comment ,c.statusid FROM chatgroups c WHERE c.cid = %s;"
            #sql = "SELECT c.cid ,c.name ,c.comment FROM chatgroups c WHERE c.cid = %s;"
    ...
    def getMessage(group_name):
        try:
            conn = DB.getConnection()
            cur = conn.cursor()
            #sql = "SELECT m.mid ,m.user_id ,m.group_id ,m.creatdate ,m.message FROM messages m WHERE m.group_id = %s;"
            #sql = "SELECT m.mid ,m.user_id ,m.creatdate ,m.message ,u.name FROM messages m INNER JOIN users AS u ON m.user_id = u.uid WHERE m.group_id = %s;"
